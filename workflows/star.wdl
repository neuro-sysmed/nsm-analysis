...
workflow RNAPreprocessing {
   ...
   String bamfile = "~{base_file_name}.star.bam"

   call Utils.FileRename {
      input:
      infile = Star.bam_file,
      outfile = bamfile
   }

   call BamUtils.Index as Index {
      input:
      basename = base_file_name
   }


   call QC.CollectRnaSeqMetrics as CollectRnaSeqMetrics {
      input:
         input_bam = bamfile,
#         output_bam_prefix = "~{base_file_name}.readgroup",
  ...
  output {
      ...
      File output_bam = "~{base_file_name}.star.bam"
      File output_bam_index = "~{base_file_name}.star.bam"
      File output_bam_md5 = "~{base_file_name}.star.bam.md5"
